...
test('It can fetch a category stream', t => {
  const category = makeCategory()
  const otherCategory = makeCategory()
  const eventCountProjection = {
    $init: 0,
    Test: (state, event) => state + 1
  }

  const stream1 = `${category}-${uuid()}`
  const stream2 = `${category}-${uuid()}`
  const stream3 = `${otherCategory}-${uuid()}`

  const testMessage = () => ({ id: uuid(), type: 'Test', data: {} })

  return reset()
    .then(() => config.messageStore.write(stream1, testMessage()))
    .then(() => config.messageStore.write(stream1, testMessage()))
    .then(() => config.messageStore.write(stream2, testMessage()))
    .then(() => config.messageStore.write(stream3, testMessage()))
    .then(() =>
      config.messageStore
        .fetch(category, eventCountProjection)
        .then(projected => {
          t.equals(projected, 3, 'Only projected the category in question')
        })
    )
})
